...
- **GitHub Copilot**: Supports `session_start`, `session_end`, `pre_tool_use`, `post_tool_use`, `pre_prompt`, `pre_compact`, `subagent_start`, `subagent_stop`, and `agent_stop`. Written to `.github/hooks/*.json`.
- **Other Editors**: Zed, Codex, Gemini, and OpenCode do not support hooks. aix will warn if you define hooks that aren't supported by your target editors.
